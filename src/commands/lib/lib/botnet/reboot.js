...
CommandsCore.cmdInit(/^reboot$/i, async (client, command, args) => {
    ...
    for(const server of CommonData.servers[client.user.network]) {
        ...
        const res = await Botnet.reboot(server);
        if(!res) {
    ...
    client.write(`\tИнициирована перезагрузка серверов.`.bgBlue.brightWhite + '\n\n'
